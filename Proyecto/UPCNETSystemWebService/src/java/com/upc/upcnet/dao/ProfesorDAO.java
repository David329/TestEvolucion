...
public class ProfesorDAO {
    ...
    public List<HorarioProfesor> getHorarioProfesor(String _idProfesor){
        ...
        try{
            ...
            while(rs.next()){
                ...
                hp.setHoraIni(rs.getDate("HoraIni"));
                hp.setHoraFin(rs.getDate("HoraFin"));
    ...
    public List<AlumnosEnCurso> getAlumnosEnCurso(String _idCurso){
        ...
        try{
            cn = AccesoDB.getConnection();
            StringBuilder query = new StringBuilder();
            query.append("SELECT C.IDCurso, c.Nombre AS CursoN, a.Nombre AS AlumnoNombre, a.Apellido AS AlumnoApellido, ca.PC1, ca.PC2, ca.EP, ca.EF  FROM Curso c JOIN Curso_Alumno ca ON ca.IDCurso = c.IDCurso JOIN Alumno a ON a.IDAlumno = ca.IDAlumno WHERE c.IDCurso = ?");
            PreparedStatement ps = cn.prepareStatement(query.toString());
